console.log("moreinfo frame ******" + window.location.href)
...
waitForElm('#content').then((elm) => {
    ...
    if (h3_title) {
        ...
        if (h3_title.innerText === "Summary of Requirements for the Degree") {
            ...
            const link_open_tab = document.createElement('span');
            link_open_tab.href = "#"
            link_open_tab.textContent = "open as a new tab ↗ ↗️"
            link_open_tab.onclick = function () {
                chrome.runtime.sendMessage({
                    action: "openNewTab",
                    url: 'page.html',
                    content: document.body.innerHTML,
                    content_head: document.head.innerHTML// or build url dynamically here
                });

            };
            content_div.insertAdjacentElement('afterbegin', summary_div);
            content_div.insertAdjacentElement('afterbegin', link_open_tab);
        }
    }
